...
const MemoizedCell = React.memo(({ rectProps, data, activeCurrencyCode }) => {
  ...
  let fill = '#3f3f46';
  if (count > 0 && count <= 20) fill = '#86efac';
  else if (count > 20 && count <= 100) fill = '#22c55e';
  else if (count > 100) fill = '#14532d';
  ...
  return (
            ...
            <>
              <p className="text-white/80">
                ...
                <span className="text-emerald-400 font-semibold">
                  {formatCurrency(data.count, activeCurrencyCode)}
                </span>
              </p>
    ...
    >
      <rect
        ...
        rx={2}
        ry={2}
        className="hover:stroke-white hover:stroke-[1px] transition-all cursor-pointer"
      />
    </Tooltip>
  );
});
...
export const SpendingHeatmap = () => {
  const { expenses = [], currentViewCurrency, getConversionRate } =
    useExpenseStore();
      ...
      <div className="w-full overflow-x-auto scrollbar-hide">
        <div className="min-w-[750px]">
          <HeatMap
            ...
            style={{
              color: '#adbac7',
              '--theme-heatmap-color': 'transparent',
            }}
            startDate={startDate}
            endDate={endDate}
            legendCellSize={0}
